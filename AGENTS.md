...
10. **Hardware Quirks & Indoor Spoofing (Phase 2):**
    - **MPU-6500 Clones:** Many cheap GY-521 modules ship with MPU-6500 chips instead of MPU-6050. The Adafruit library's `WHO_AM_I` check is patched to accept `0x70`. Factory offsets are severe, so `kRestingMotionThresholdG` is raised to `0.25F` and `kWalkingMotionThresholdG` to `0.50F`.
    - **DHT11 Pin Migration:** Due to dead pins on some ESP32 development boards, the DHT11 is wired to `GPIO15` (not `D4`).
    - **Indoor Spoofing:** To allow development without live cows or clear skies, `has_complete_sensor_record()` actively overrides bad MLX readings with `38.5C` and missing GPS locks with coordinates for VIT Vellore, permitting local ThingSpeak transmissions.
