...
void print_status() {
  ...
  Serial.printf(
      "[status] Tbody=%.2fC THI=%.2f behaviour=%u GPS=%.6f,%.6f risk=%u (%s) geo=%u battery=%u%% status=%s\n",
      latest_frame.body_temp_c, latest_frame.thi, latest_frame.behaviour,
      latest_frame.latitude, latest_frame.longitude, latest_frame.risk_score,
      collar_math::alert_band(latest_frame.risk_score, parity_contract::kGreenMax,
                               parity_contract::kYellowMax),
      latest_frame.geofence_status, latest_frame.battery_pct, latest_frame.status);
