...
- [ADR-024: Hardware Quirks & Indoor Spoofing (Phase 2 Debugging)](#adr-024-hardware-quirks--indoor-spoofing-phase-2-debugging)
...
## ADR-024: Hardware Quirks & Indoor Spoofing (Phase 2 Debugging)

* **Status:** Implemented / Approved
* **Date:** 2026-08-28
* **Supersedes:** Parts of ADR-003 relating to pin mappings and ADR-006 regarding strict fallback behavior.
* **Context:** During Phase 2 deployment to physical hardware, severe discrepancies were discovered in cheap GY-521 modules, GPIO failures on specific ESP32 boards, and testing bottlenecks caused by the strict GPS and MLX90614 sensor locks required to send telemetry.
* **Decision:**
  * **MPU-6500 Clone Compatibility:** The supplied GY-521 boards utilize the MPU-6500 chip rather than the authentic MPU-6050, returning a `WHO_AM_I` register value of `0x70` instead of `0x68`. The `Adafruit_MPU6050.h` library must be locally patched to `#define MPU6050_DEVICE_ID 0x70`.
  * **Motion Threshold Adjustments:** Due to massive uncalibrated factory offsets in cheap MPU clones (often exceeding 0.20G of fake deviation while resting perfectly flat), `kRestingMotionThresholdG` is raised from `0.05F` to `0.25F`, and `kWalkingMotionThresholdG` is raised from `0.15F` to `0.50F` in `device_config.h`. 
  * **Behaviour Fallback:** The previous fallback of `Unknown (5)` for values between Resting and Walking has been changed to `Grazing (1)` to provide cleaner continuous telemetry during active use without resorting to error states.
  * **DHT11 Pin Migration:** `GPIO4` was found to be unreliable/dead on the user's specific ESP32 dev board. The DHT11 `DATA` pin is permanently migrated to `GPIO15`.
  * **Indoor Spoofing:** To permit cloud testing without requiring live cows or clear outdoor skies, `has_complete_sensor_record()` actively overrides bad MLX readings with a healthy `38.5C`, and missing GPS locks with coordinates for VIT Vellore (`12.9716, 79.1589`). 
  * **Continuous Serial Telemetry:** The serial monitor format was redesigned to print a multi-line, human-readable block every 5 seconds (independent of ThingSpeak uploads) containing raw G-force deviation to aid in diagnosing uncalibrated sensors.
* **Consequences:** The firmware now perfectly accommodates low-quality hardware clones and supports fully decoupled indoor testing while successfully pushing real/spoofed data to ThingSpeak Channel 1.
